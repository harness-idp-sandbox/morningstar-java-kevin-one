...
# run app
java -jar target/java-kevin-one-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-kevin-one:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-kevin-one -n dev

# get external endpoint
kubectl get svc java-kevin-one-svc -n dev
```
...
```bash
kubectl port-forward svc/java-kevin-one-svc 8080:80 -n dev
...
kubectl rollout restart deploy/java-kevin-one -n dev
kubectl rollout status deploy/java-kevin-one -n dev
```
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javakevinone/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© kevin.niu@morningstar.com. For demo/PoV purposes.
